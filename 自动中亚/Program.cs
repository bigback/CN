...
namespace SephZhonya
{
    ...
    class Program
    {
        ...
        {
            Game.PrintChat("<font color=\"#12FA54\"><b>鑷姩涓簹 v." + Version + " by Seph</font></b>");
            Player = ObjectManager.Player;
            EventSubscriptions();
            Menu = new Menu("自动中亚--后爸爸汉化", "AutoZhonya", true);
            var spellmenu = new Menu("技能", "Spells");
            var miscmenu = new Menu("其他", "Misc");
            miscmenu.AddItem(new MenuItem("enablehpzhonya", "低血量自动中亚").SetValue(true));
            miscmenu.AddItem(new MenuItem("hptozhonya", "自动中亚血量")).SetValue(new Slider(25, 0, 100));
            miscmenu.AddItem(new MenuItem("minspelldmg", "技能伤害(非危险)")).SetValue(new Slider(45, 0, 100));
            miscmenu.AddItem(new MenuItem("remaininghealth", "保留血量")).SetValue(new Slider(15, 0, 100));
